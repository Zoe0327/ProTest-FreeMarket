...
<div class="purchases__form">
    <form method="POST" action="{{ route('purchase.store', $product->id ?? 1) }}" class="purchase-form">
        @csrf
        <div class="purchase-content-wrapper">
            <div class="purchase__item">
                <div class="purchase-item__img">
                    <img src="{{ $product->image_url ?? 'https://placehold.co/120x120/eee/333?text=商品画像' }}" alt="商品画像" class="item-summary__image">
                    <div class="purchase-item__detail">
                        <h2 class="purchase-item__name">{{ $product->name ?? '商品名' }}</h2>
                        <p class="purchase-price">￥{{ number_format($product->price ?? 47000) }}</p>
                    </div>
                </div>
                ...
                <div class="purchase-item__payment">
                    ...
                    <select name="payment_method_id" id="payment_method_id" class="payment-select">
                ...
                <div class="purchase-item__address">
                    <div class="address__header">
                        ...
                        <a href="{{ route('purchase.address.edit', $product->id ?? 1) }}" class="address-edit">変更する</a>
                    </div>
                    <!-- 実際にはDBから取得した配送先情報を表示 -->
                    <div class="confirmed-address">
                        <p class="address-zip">〒123-4567</p>
                        <p class="address-full">ここには住所と建物が入ります</p>
                    </div>
                </div>
                <!-- 区切り線用のダミーdiv (CSSでボーダーを表現) -->
                <div class="separator"></div>
            ...
            <div class="purchase__item-confirmation">
                <div class="confirmation-form">
                    ...
                    <div class="confirmation-row">
                        ...
                        <span class="row-value">￥{{ number_format($product->price ?? 47000) }}</span>
                    </div>
                    <div class="confirmation-row">
                        <span class="row-label">支払い方法</span>
                        <span class="row-value" id="displayed_payment_method">コンビニ払い</span>
                    </div>
                    <div class="confirmation-row confirmation-row--total">
                        <span class="row-label">支払い金額</span>
                        <span class="row-value total-price">￥{{ number_format($product->price ?? 47000) }}</span>
                    </div>
                </div>
...
<script>
    ...
    function updatePaymentMethod() {
        ...
        if (selectElement && displayedPaymentMethod) {
            ...
            displayedPaymentMethod.textContent = selectElement.options[selectElement.selectedIndex].text;
        }
    }
</script>
@endsection